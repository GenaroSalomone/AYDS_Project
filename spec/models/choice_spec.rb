...
describe Choice do
  ...
  describe 'validations' do
    ...
    it "is invalid without text" do
      choice = Choice.new(type: 'choice_type', difficulty_id: 1, help: 'choice_help')  
      expect(choice.valid?).to eq(false)
    end

    it "is invalid without type" do
      choice = Choice.new(text: 'choice_text', difficulty_id: 1, help: 'choice_help')
      expect(choice.valid?).to eq(false)
    end

    it "is invalid without difficulty_id" do
      choice = Choice.new(text: 'choice_text', type: 'choice_type', help: 'choice_help')
      expect(choice.valid?).to eq(false)
    end

    it "is invalid with empty help for beginner difficulty" do
      choice = Choice.new(text: 'choice_text', type: 'choice_type', difficulty_id: 1, help: '')
      expect(choice.valid?).to eq(false)
    end
